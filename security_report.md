...
> [!IMPORTANT]
> Tài liệu này mô tả chi tiết các chốt chặn bảo mật, phân quyền và chiến lược chống tấn công (Từ Botnet đến AI) hiện đang được áp dụng trên Fast Platform, đặc biệt tập trung vào phân khu Quản trị (`admin.micsmo.com`) và Bảo mật Dữ liệu Khách hàng tại Storefront.

---

## 🛡️ 1. Bảo Mật Cấp Admin (`admin.micsmo.com`)
...
- **Kiểm soát Origin Tuyệt đối:** Middleware `DomainGuardMiddleware` giám sát chặt chẽ Header `Host` và `X-Forwarded-Host`. Mọi truy cập vào các API đặc quyền (`ADMIN_ONLY_PREFIXES` như `/api/v1/users`, `/api/v1/settings`) **bắt buộc** phải xuất phát từ `admin.micsmo.com` hoặc `api.micsmo.com`.
- **Chặn Đột Biến Dữ Liệu (Mutation Lockdown):** Các tài nguyên dùng chung như Sản phẩm, Bài viết chỉ cho phép Storefront (`micsmo.com`) thực thi lệnh Đọc (GET). Mọi thao tác Ghi/Sửa/Xóa (`POST`, `PATCH`, `PUT`, `DELETE`) bị chặn đứng nếu không đến từ Admin Domain.
...
- **Cookie Bảo Mật Kép:** Access Token được thiết lập qua Cookie với cờ `HttpOnly`, `Secure` và `SameSite=Lax`, bị ràng buộc cứng với Root Domain (`.micsmo.com`) để chặn XSS (Cross-Site Scripting).
