  ...
  - **Xử lý (Phương án tối ưu hóa siêu tốc):**
    ...
    - **Sửa lỗi hiển thị tiêu đề kỹ thuật và thiếu CTA bán hàng:**
      ...
      - *Xử lý:*
        ...
        - Tinh chỉnh `SYSTEM_PROMPT` trong `ConsultantHandler` (`consultant.py`) để yêu cầu Helen bắt buộc báo giá bán chi tiết (giá niêm yết + khuyến mãi), thông tin tồn kho thực tế (FOMO), và đưa ra Kêu Gọi Hành Động (CTA) thu thập SĐT + Địa chỉ của khách để đóng gói gửi ưu đãi ngay lập tức.
