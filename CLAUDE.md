...
> **CHỈ THỊ TỐI CAO:** Dự án Agentic AI 2026. Stack: **SvelteKit 5 (Runes: $state, $derived, $effect, $props) + Litestar (Python 3.14) + SQLAlchemy 2.0 + PydanticAI +V2(only) + LiteLLM**. Ép kiểu tĩnh 100% (CẤM 'any'), Cấm tuyệt đối Svelte 4 (writable hoặc readable stores). Hoạt động trên domain `*.micsmo.com`.
> > ** CẤM GIẢI THÍCH, LÀM XONG BÁO KẾT QUẢ.**

## 🛑 R00 – KỶ LUẬT TÁC CHIẾN (WAR ROOM PROTOCOL)

- ❌ **CẤM Tuyệt đối:** Hardcode giá trị logic, dùng dữ liệu giả (Mock Data/Placeholders) hoặc code `// TODO`. Mọi dữ liệu phải flow từ DB/API hoặc Config chính thống. Client giao tiếp dữ liệu phải thông qua api. Câm tuyệt đối thao tác trực tiếp với database.
- ✅ **FILE SIZE:** mỗi file không quá 500 dòng.

## 🔍 R01 – TRINH SÁT (SCOUT PROTOCOL)
- ✅ **PHÁT HIỆN DỊ THƯỜNG:** Mọi bug, code thối hoặc điểm tối ưu, các kỹ thuật sai, lỗi thời phát hiện được trong lúc đọc file phải được báo cáo ở cuối luồng phân tích.
...
- 🧪 **TRINH SÁT CÔNG NGHỆ:** AI không được dậm chân tại chỗ. Phải luôn chủ động tìm kiếm các kỹ thuật lập trình, thuật toán và giải pháp kiến trúc mới nhất/tầng cao nhất, phân tích các viral công nghệ hot đề xuất, phản biện và áp dụng vào kế hoạch báo cáo.
...
- **Resource Discipline:** Phải "Dispose" ngay tài nguyên (WebSocket/SSE) khi xong để bảo vệ 4GB RAM.
...
- ✅ **PYDANTIC V2 ONLY:** CẤM dùng Pydantic V1 hoặc `pydantic.v1`. Mọi schema phải dùng Pydantic V2.
