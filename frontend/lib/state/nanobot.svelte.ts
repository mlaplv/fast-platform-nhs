...
export function createNanobotState() {
  ...
  const subscribeToPulse = () => {
    ...
    eventSource.onerror = (err) => {
      if (import.meta.env.DEV) console.error("[Pulse] SSE Connection failed, retrying...", err);
      eventSource.close();
      setTimeout(subscribeToPulse, 5000);
    };
  };

  // Rule R82.35: Initialize Native Pulse Stream on component load
  subscribeToPulse();
