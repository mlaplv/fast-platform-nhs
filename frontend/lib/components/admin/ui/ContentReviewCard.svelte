<script lang="ts">
  ...
  let step = $state(incomingStep);
  let status = $state(incomingStatus);
  let progress_msg = $state(incomingProgressMsg);
  
  // keywords is an object like { title: "...", primary_keyword: "...", secondary_keywords: [...] }
  let keywords = $state(incomingKeywords || {});
  let assets = $state(incomingAssets || []);
  let outline = $state(incomingOutline || []);
  
  // Secondary State for Editing (Deep Copy)
  let editedKeywords = $state({ ...incomingKeywords });
