...
<div class="flex-1 flex flex-col pt-0">
  <div class="flex items-start gap-4 mb-2">
    <h1
      class="text-[24px] font-bold text-gray-900 leading-[1.2] tracking-tight"
    >
    ...
    <div class="flex items-center gap-1.5">
      {#if soldStr}
        <span class="text-black font-black text-[16px]"
          >{soldStr.includes("đã bán") || soldStr.includes("Đã bán")
            ? soldStr.replace(/[Đđ]ã bán\s*/g, "")
            : soldStr}</span
        >
  ...
  <!-- Price Bar -->
  <div
    class="bg-[#f6f6f6] px-5 py-1.5 flex items-center justify-between mb-2 relative overflow-hidden group border-y border-gray-100/50"
  >
    <div class="flex flex-col">
      <div class="flex items-center gap-3 mb-0.5">
        {#if activePrices.original > activePrices.sale}
          ...
          >
          <span
            class="text-[11px] font-black text-[#00bfa5] tracking-widest bg-[#e6f9f6] px-1.5 py-0.5"
          >
      ...
      <div class="flex items-baseline gap-4">
        <span
          class="text-[32px] font-black text-[#d0011b] tracking-tighter leading-none"
      ...
      <div class="mt-1 flex flex-col gap-2">
        <div class="flex items-center gap-2.5">
          {#if activeComboQty > 1}
            <div
              class="bg-slate-900 text-white text-[8px] font-black px-1.5 py-0.5 rounded-sm tracking-widest flex items-center gap-1"
            >
              <Package size={10} class="text-white/70" />
          ...
          <div class="flex items-center gap-1.5 group/ai cursor-default">
            ...
            <span
              class="text-[8px] text-blue-500 font-mono font-black tracking-[0.2em]"
              >Helen</span
            >
            <div
        ...
        <div class="relative pl-4 border-l border-blue-200/40">
          <p
            class="text-[12.5px] text-slate-500 font-medium leading-[1.4] max-w-[580px] tracking-tight"
          >
            {helenAdvice}
          </p>
        </div>
    ...
    {#if isFlashSaleActive}
      <div class="flex flex-col items-end">
        ...
        <div
          class="flex gap-1 text-gray-800 font-black text-[17px] font-mono tabular-nums select-none"
        >
          <div
            class="bg-gray-200/50 px-1.5 py-0.5 min-w-[30px] text-center rounded-sm"
          >
            {timeLeft.hours < 10 ? "0" + timeLeft.hours : timeLeft.hours}
          </div>
          <span class="opacity-30 self-center w-1.5 text-center text-[12px]"
            >:</span
          >
          <div
            class="bg-gray-200/50 px-1.5 py-0.5 min-w-[30px] text-center rounded-sm"
        ...
        {#each productVouchers as v}
          ...
          >
            ...
            <div class="flex flex-col items-start translate-x-1">
              ...
              >
              <span
                class="text-[9px] text-gray-400 font-bold mt-1 tracking-tighter inline-block first-letter:uppercase"
                >{v.sub || ""}</span
              >
            </div>
            {#if isApplied}
  ...
  {#if variations.length > 0}
    <div class="space-y-5 mb-3 mt-1">
      {#each variations as tier, tIdx}
        <div class="flex items-start">
          <span
            class="w-[70px] shrink-0 text-[14px] text-gray-500 mt-2 capitalize"
            >{tier.name}</span
          >
                ...
                >
                  {#if tIdx === 0 && tier.images?.[oIdx]}
                    <img
                      ...
                      alt={option}
                      class="w-6 h-6 object-cover mr-2 border border-gray-100"
                    />
                  {/if}
  ...
  <div class="flex items-center mb-2">
    <span
      class="w-[70px] shrink-0 text-[12px] font-bold text-gray-400 tracking-widest"
      >Số lượng</span
    >
    <div class="flex items-center gap-8">
      <div
        class="flex items-center border border-gray-100 rounded-none h-9 group overflow-hidden bg-white shadow-sm"
      >
        <button
          type="button"
      ...
      <div class="flex flex-col gap-0.5">
        <span
          class="text-[12px] text-gray-900 font-black tracking-tighter italic"
          >Số lượng có hạn</span
        >
        {#if currentStock < 10}
          <span
        ...
        {:else}
          <span
            class="text-[11px] text-[#00bfa5] font-black tracking-tight italic"
            >Đang có sẵn tại kho Mall chính hãng</span
          >
        {/if}
      </div>
    </div>
        ...
        <div class="flex items-start gap-4 relative z-10">
          ...
          <div class="flex-1 space-y-3">
            <div class="flex items-center justify-between">
              <h3 class="text-[14px] font-black tracking-widest text-gray-800">
                Ưu đãi độc quyền
              </h3>
              {#if activeComboQty > 1}
                <div
                  class="bg-[#d0011b] text-white text-[10px] font-black px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-md"
                >
