...
<svelte:element this={outerWrapper} class="bg-white min-h-screen text-gray-900">
  ...
  {#if article.metadata?.how_to && article.metadata.how_to.steps && article.metadata.how_to.steps.length > 0}
    <div class="px-4 py-8 bg-gradient-to-tr from-gray-50/70 to-white border-t border-gray-100/60">
      <!-- Header -->
      <div class="mb-5 flex items-center gap-2">
        <span class="w-1.5 h-1.5 bg-[#C18F7E] rounded-full animate-pulse"></span>
        <h2 class="text-[16px] font-black text-gray-900 tracking-tight uppercase" style="margin: 0; border: none; padding: 0;">Hướng dẫn thực hiện từng bước</h2>
      </div>
      
      <!-- Quick Meta (Total time, tools, supplies) -->
      <div class="space-y-3 mb-6 bg-[#fff0f0]/60 p-4 border-l-4 border-[#C18F7E] shadow-[10px_10px_30px_rgba(193,143,126,0.02)]">
        <div>
          <span class="block text-[9px] font-black text-gray-400 tracking-wider uppercase mb-0.5">Thời gian thực hiện</span>
          <span class="text-xs font-bold text-gray-800 flex items-center gap-1.5">
            <svg class="w-3.5 h-3.5 text-[#C18F7E]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        ...
        {#if article.metadata.how_to.tools && article.metadata.how_to.tools.length > 0}
          <div>
            <span class="block text-[9px] font-black text-gray-400 tracking-wider uppercase mb-0.5">Dụng cụ cần thiết</span>
            <span class="text-xs font-bold text-gray-800 flex flex-wrap gap-1.5">
        ...
        {#if article.metadata.how_to.supplies && article.metadata.how_to.supplies.length > 0}
          <div>
            <span class="block text-[9px] font-black text-gray-400 tracking-wider uppercase mb-0.5">Nguyên liệu / Sản phẩm</span>
            <span class="text-xs font-bold text-[#C18F7E] flex flex-wrap gap-1.5">
      ...
      <div class="space-y-4">
        {#each article.metadata.how_to.steps as step, i}
          <div class="bg-white border border-gray-100 p-4 rounded-xl shadow-sm flex gap-3">
            <div class="flex-shrink-0 w-8 h-8 bg-[#C18F7E] text-white font-black text-sm flex items-center justify-center rounded-full shadow-sm shadow-[#C18F7E]/30">
              {i + 1}
            </div>
            <div class="flex-1">
              <h3 class="text-sm font-black text-gray-900 mb-0.5" style="margin: 0; padding: 0;">{step.name}</h3>
              <p class="text-xs text-gray-600 leading-relaxed" style="margin: 0.15rem 0 0 0;">{step.text}</p>
              {#if step.image}
                <img src={step.image} alt={step.name} class="mt-3 rounded-lg max-h-[180px] w-auto object-cover" />
              {/if}
            </div>
          </div>
        {/each}
