...
<svelte:element this={outerWrapper} class="bg-white min-h-screen text-gray-900">
  ...
  <div class="h-[65vh] relative w-full overflow-hidden bg-black">
    <ImageWithFallback src={article.featuredImage} alt={article.title} aspectRatio="aspect-video" class="absolute inset-0 w-full h-full z-0 opacity-90 scale-105" />
    ...
    <div class="absolute top-0 left-0 w-full px-[5px] py-4 z-30 flex justify-between items-center">
        <button onclick={() => history.back()} class="w-10 h-10 bg-black/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/10 active:scale-90 transition-all">
            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7" /></svg>
        </button>
        <button class="w-10 h-10 bg-black/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-white/10 active:scale-90 transition-all">
            <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
        </button>
    </div>

    <!-- Viral Content Overlay -->
    <div class="absolute bottom-0 left-0 w-full px-[5px] pb-10 z-20 flex flex-col items-start gap-4">
        <div class="flex items-center gap-2">
          <span class="bg-[#C18F7E] text-white px-3 py-1.5 text-[9px] font-black tracking-widest rounded-full shadow-lg shadow-black/20 flex items-center gap-1.5 border border-white/10">
            <span class="text-[12px]">💡</span> {article.category || 'HƯỚNG DẪN CHUYÊN MÔN'}
          </span>
          <span class="bg-white/10 backdrop-blur-md text-white/80 px-3 py-1.5 text-[9px] font-black tracking-widest rounded-full border border-white/5">
            ⏱️ 5 PHÚT ĐỌC
          </span>
        </div>
        
        <h1 class="text-[32px] font-black text-white leading-[1.1] tracking-tight drop-shadow-2xl">
          {formattedTitle}
        </h1>

        <div class="flex items-center gap-3 mt-2">
            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-[#C18F7E] to-black border-2 border-white/20 flex items-center justify-center text-white text-[10px] font-black">
  ...
  <div class="py-10 border-t border-gray-100">
    <h2 class="px-[5px] text-[12px] font-black tracking-[0.2em] text-[#0f172a] mb-6 flex items-center gap-3">
      ...
      {#each article.metadata.faqs as faq, i}
        <div class="bg-white">
          <button 
            ...
            onclick={() => toggleFaq(i)}
          >
            <span class="text-[14px] font-bold {activeFaq === i ? 'text-[#C18F7E]' : 'text-[#0f172a]'} pr-4 leading-snug transition-colors">{faq.question}</span>
            <svg 
              class="w-4 h-4 text-[#C18F7E] shrink-0 transition-transform duration-300 {activeFaq === i ? 'rotate-180' : ''}" 
