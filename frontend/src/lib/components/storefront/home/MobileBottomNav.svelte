<script lang="ts">
  ...
  const hotline = $derived(ui.settings?.contact_info?.hotline || ui.settings?.hotline || '0949901122');
</script>
...
>
  <div class="tbn-nav-inner {isProductMode ? 'tbn-nav-inner--product' : ''}">
    ...
    <button class="tbn-item" aria-label="Hotline" onclick={() => window.location.href = `tel:${hotline.replace(/\D/g, '')}`}>
