<script lang="ts">
  ...
  let seconds = $state(Number(product?.metadata?.flash_sale_seconds) || (15 * 3600 + 54 * 60 + 16)); 
  ...
  const displayProduct = $derived(() => {
    ...
    const currentPrice = product.discountPrice || product.discount_price || product.price;
...
{#if displayProduct()}
  ...
  <div class="category-banner relative h-[400px] md:h-[450px] overflow-hidden bg-white/50 backdrop-blur-3xl border-b border-black/[0.03]">
    ...
    <div in:fade={{duration: 800}} class="absolute inset-0 flex items-center justify-between px-8 md:px-[76px] z-10">
      <!-- Content Left -->
        ...
        <div in:fly={{y: 30, duration: 1000, delay: 500}} class="flex items-center gap-8">
            <div class="flex flex-col">
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <span class="discount-tag animate-pulse">−{slide.discountPercent}%</span>
                </div>
            ...
            <div class="flex flex-col">
                ...
                <span class="sold-count italic">+{slide.sales?.toLocaleString()}</span>
            </div>
        </div>
      ...
      <div class="absolute right-0 top-0 w-[45%] h-full flex items-center justify-center pointer-events-none">
        <div in:scale={{duration: 1500, start: 0.9, easing: backOut, delay: 400}} class="relative h-[85%] w-[85%] flex items-center justify-center">
            ...
            {#each specs() as spec, idx}
                ...
                <div in:fly={{x: 30, duration: 1000, delay: 800 + (idx * 200)}} 
                     class="spec-badge shadow-xl animate-float-spec {spec.isVerification ? 'cursor-pointer hover:scale-105 transition-transform' : ''}" 
                     style="top: {20 + (idx * 20)}%; right: {5 + (idx % 2 * 8)}%; animation-delay: {idx * 1.5}s"
                     onclick={() => spec.isVerification && (showVerificationModal = true)}>
                    <span style="font-size: 7px; font-weight: 900; letter-spacing: 0.1em; color: rgba(0,0,0,0.4)">{spec.label}</span>
                    <span style="font-size: 9px; font-weight: 900;" style:color={spec.color}>{spec.value}</span>
                </div>
            {/each}

            <div in:fly={{x: -30, duration: 1000, delay: 1200}} class="origin-badge shadow-xl">
            ...
            <img src={slide.image} alt={slide.name} class="product-image filter drop-shadow-2xl animate-float-3d" />
        </div>
      </div>
    </div>
  </div>
{:else}
  <div class="category-banner relative h-[400px] md:h-[450px] overflow-hidden bg-white/50 backdrop-blur-3xl border-b border-black/[0.03] flex items-center justify-center">
      ...
      <div class="relative z-10 flex flex-col items-center justify-center opacity-20 select-none" in:fade={{duration: 800}}>
          <h2 class="text-6xl md:text-8xl font-black italic tracking-tighter text-gray-500 bg-gradient-to-br from-gray-400 to-gray-200 bg-clip-text text-transparent">
              osmo.vn
          </h2>
      </div>
  </div>
...
{#if showVerificationModal && displayProduct()}
  <!-- svelte-ignore a11y_click_events_have_key_events -->
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div use:portal transition:fade={{duration: 200}} class="fixed inset-0 flex items-center justify-center p-4 sm:p-8 bg-black/80 backdrop-blur-xl" style:z-index={Z_INDEX_CLIENT.MODAL + 50} onclick={() => showVerificationModal = false}>
    <div transition:scale={{duration: 300, start: 0.95}} class="relative max-w-5xl w-full h-auto max-h-[90vh] overflow-y-auto bg-[#0a0a0a] border border-white/10 shadow-[0_20px_100px_rgba(0,0,0,1)] rounded-xl no-scrollbar" onclick={(e) => e.stopPropagation()}>
      <button 
...
<style>
  ...
  .original-price { @apply text-sm font-bold text-gray-300 line-through decoration-gray-400/20; }
  .discount-tag { @apply text-[9px] text-[#ee4d2d] font-black tracking-widest; }
  .current-price { @apply text-[#ee4d2d] text-3xl font-black tracking-tighter flex items-end gap-1; }
  .sold-label { @apply text-[8px] font-black tracking-[0.2em] text-black/20 mb-1; }
  .sold-count { @apply text-black text-lg font-black; }
  ...
  /* Badges & Right Image */
  .image-bg-glow { @apply absolute inset-0 bg-[#ee4d2d]/5 blur-[80px] rounded-full animate-pulse; }
  .spec-badge {
  ...
  .origin-dot { @apply h-1.5 w-1.5 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]; }
  .origin-text { @apply text-[8px] font-black tracking-[0.2em]; }

  .product-image { @apply relative z-10 max-h-full object-contain filter; }
