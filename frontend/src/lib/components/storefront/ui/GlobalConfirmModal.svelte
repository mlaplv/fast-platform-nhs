...
{#if modal}
  <!-- Backdrop -->
  <div 
    class="fixed inset-0 bg-black/80 backdrop-blur-xl transition-all z-[calc(var(--z-modal)-1)]"
  ...
  >
    <div 
      class="w-full max-w-md bg-[#0a0a0a] border border-white/5 shadow-2xl pointer-events-auto overflow-hidden relative"
    ...
    >
      ...
      <div class="relative z-10 p-8">
        <!-- Header -->
        <h3 class="text-xs font-black uppercase tracking-[0.3em] text-[#ee4d2d] mb-4 flex items-center gap-2">
        ...
        <div class="grid grid-cols-2 gap-4">
          <button 
            onclick={modal.onCancel}
            class="py-4 px-6 border border-white/10 text-white font-black text-[10px] uppercase tracking-widest hover:bg-white/5 transition-all text-center"
          >
            {modal.cancelLabel}
          </button>
          ...
          <button 
            ...
            class="py-4 px-6 bg-[#ee4d2d] text-white font-black text-[10px] uppercase tracking-widest hover:brightness-110 transition-all text-center relative group overflow-hidden"
          >
            <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
            <span class="relative z-10">{modal.confirmLabel}</span>
          </button>
        </div>
      </div>
    </div>
...
<style>
  /* Elite V2.2: Liquid Glass Texture */
  div {
