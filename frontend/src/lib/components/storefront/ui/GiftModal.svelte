<script lang="ts">
  ...
  let { isOpen = $bindable<boolean | undefined>(undefined), onClose } =
    $props();
  ...
  const showModal = $derived(
    isOpen !== undefined ? isOpen : activeStore?.isGiftModalOpen,
  );

  const giftPresets = [
    {
      label: "Sinh nhật",
      text: "Chúc mừng sinh nhật! Chúc bạn luôn xinh đẹp và hạnh phúc rạng ngời. ❤️",
    },
    {
      label: "Tặng Vợ",
      text: "Tặng vợ yêu món quà bất ngờ. Cảm ơn em đã luôn đồng hành cùng anh! 😘",
    },
    {
      label: "Kỷ niệm",
      text: "Mừng ngày kỷ niệm của chúng ta. Yêu em nhiều hơn mỗi ngày. 🥂",
    },
    {
      label: "Xin lỗi",
      text: "Đừng giận anh nữa nhé, món quà nhỏ thay lời muốn nói. 💐",
    },
  ];
  ...
  $effect(() => {
    if (showModal && activeStore?.giftInfo) {
      ...
      form.recurringMetadata = activeStore.giftInfo.recurring_metadata || {
        daysOfWeek: [],
        dayOfMonth: 1,
      };
    }
  ...
  function save() {
    ...
    activeStore.setGiftInfo({
      ...
      recurring_metadata:
        form.recurringType !== "none" ? form.recurringMetadata : undefined,
    });
  ...
  >
    ...
    >
      ...
      <div class="flex items-center justify-between p-6 pb-2 relative z-10">
        <div>
          <h3
            class="text-xl font-black text-white italic tracking-wider flex items-center gap-2"
          >
            <span class="text-2xl">🎁</span> BẠN LÀ NGƯỜI TẶNG QUÀ?
          </h3>
          <p
            class="text-[10px] text-pink-400 font-bold tracking-widest mt-1 opacity-80"
          >
            Thông tin người gửi
          </p>
        </div>
      ...
      <!-- Scrollable Content -->
      <div
        class="p-6 pt-4 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar relative z-10"
      >
        <!-- Recipient Info Grid -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="field-wrap-viral">
            <input
              ...
              placeholder=" "
              class="field-input-viral"
              id="v-name"
            />
            <label for="v-name" class="field-label-viral"
              >Tên của bạn (Để in thiệp)</label
            >
          </div>
          <div class="field-wrap-viral">
            <input
              type="tel"
              bind:value={form.senderPhone}
              placeholder=" "
              class="field-input-viral"
              id="v-phone"
            />
            <label for="v-phone" class="field-label-viral"
              >SĐT của bạn (Để nhận xác nhận)</label
            >
          </div>
        </div>
        ...
        <div class="space-y-2">
          <span
            class="text-[9px] font-black text-pink-500/80 tracking-widest ml-1"
            >Chọn mẫu lời nhắn nhanh</span
          >
          <div class="flex flex-wrap gap-2">
            {#each giftPresets as preset}
              <button
                type="button"
                class="preset-pill {form.message === preset.text
                  ? 'preset-active'
                  : ''}"
        ...
        <div class="field-wrap-viral">
          <textarea
            ...
            placeholder=" "
            class="field-input-viral h-28"
            id="v-msg"
          ></textarea>
          <label for="v-msg" class="field-label-viral"
            >Lời nhắn gửi yêu thương...</label
          >
        </div>
        ...
        <div class="space-y-6">
          <div class="flex items-center gap-2 mb-2">
            <span class="text-[10px] font-black text-sky-400 tracking-widest"
              >Hẹn giờ & Lặp lại (Đặc quyền)</span
            >
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-4">
              <div class="flex flex-wrap gap-2">
                <button
                  ...
                  class="preset-pill !border-sky-500/30 hover:!border-sky-500 {form.scheduledAt ===
            ...
            <div class="space-y-4">
              <div class="flex items-center justify-between px-1">
                <span
                  class="text-[9px] font-black text-slate-500 tracking-widest"
                  >Tần suất lặp lại</span
                >
                {#if form.recurringType !== "none"}
                  <span
                    class="text-[8px] font-bold text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded animate-pulse"
                    >Đang kích hoạt</span
                  >
                {/if}
              </div>
              ...
              <div class="flex flex-wrap gap-2">
                {#each repeatOptions as opt}
                  <button
                    type="button"
                    class="preset-pill {form.recurringType === opt.value
                      ? 'preset-active !border-sky-500 !bg-sky-500/10 !text-sky-400'
                      : ''}"
              ...
              {#if form.recurringType === "weekly"}
                <div class="detail-panel" in:fly={{ y: 10 }}>
                  <span class="text-[8px] font-black text-slate-500 mb-2 block"
                    >Chọn ngày trong tuần</span
                  >
                  <div class="flex gap-2">
                    {#each ["CN", "T2", "T3", "T4", "T5", "T6", "T7"] as day, i}
                      <button
                        type="button"
                        class="w-10 h-10 rounded-full border border-white/10 text-[10px] font-black transition-all {form.recurringMetadata.daysOfWeek.includes(
                          i,
                        )
                        ...
                        onclick={() => {
                          if (form.recurringMetadata.daysOfWeek.includes(i)) {
                            form.recurringMetadata.daysOfWeek =
                              form.recurringMetadata.daysOfWeek.filter(
                                (d) => d !== i,
                              );
                          } else {
                            form.recurringMetadata.daysOfWeek = [
              ...
              {:else if form.recurringType === "monthly"}
                <div class="detail-panel" in:fly={{ y: 10 }}>
                  <span class="text-[8px] font-black text-slate-500 mb-3 block"
                    >Chọn ngày trong tháng</span
                  >
                  <div class="grid grid-cols-7 gap-1">
                    {#each Array.from({ length: 31 }, (_, i) => i + 1) as d}
                      <button
                        type="button"
                        class="h-8 rounded md:rounded-lg text-[10px] font-bold transition-all {form
                          .recurringMetadata.dayOfMonth === d
      ...
      <!-- Action Footer -->
      <div
        class="p-6 bg-white/5 border-t border-white/5 flex items-center justify-between relative z-10"
      >
        <button
          onclick={close}
