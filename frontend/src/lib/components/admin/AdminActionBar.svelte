...
{#if isAdmin}
  <div 
    class="admin-action-bar fixed bottom-10 left-1/2 z-[9999] flex items-center gap-2 p-2 bg-slate-950/80 backdrop-blur-3xl border border-white/10 rounded-full shadow-[0_30px_100px_rgba(0,0,0,0.8)] {isDragging ? 'is-dragging select-none' : ''}"
  ...
  >
    ...
    <div 
      class="pl-4 pr-1 py-2 flex items-center gap-2 cursor-grab active:cursor-grabbing hover:text-blue-400 transition-colors group"
    ...
    <!-- Edit Toggle -->
    <button 
      onclick={() => liveEditStore.toggleEditMode()}
      class="flex items-center gap-3 px-6 py-3 rounded-full transition-all duration-500 {liveEditStore.isEditMode ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.1)]' : 'hover:bg-white/5 text-white/60 hover:text-white'}"
    >
      <Edit3 size={16} />
      <span class="text-[11px] font-black uppercase tracking-[0.2em]">{liveEditStore.isEditMode ? 'DANG CHỈNH SỬA' : 'CHẾ ĐỘ CHỈNH SỬA'}</span>
    </button>
    ...
    {#if liveEditStore.isEditMode}
      ...
      <button 
        ...
        class="flex items-center gap-3 px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-full transition-all duration-300 shadow-[0_10px_40px_rgba(37,99,235,0.3)] active:scale-95 disabled:opacity-50"
      >
        ...
        {:else}
          <Save size={16} />
        {/if}
        <span class="text-[11px] font-black uppercase tracking-[0.2em]">{liveEditStore.isSaving ? 'DANG LƯU...' : 'LƯU THAY ĐỔI'}</span>
      </button>
