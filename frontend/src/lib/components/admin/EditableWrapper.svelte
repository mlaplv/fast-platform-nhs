<script lang="ts">
  ...
  const isDisabled = $derived(String(currentValue).startsWith('-'));

  // Logic chặn render: Chỉ render nếu KHÔNG bị disable HOẶC đang ở chế độ Edit
  ...
  const getValue = (p: string, fallback = ""): string => {
    ...
    try {
        const keys = p.split(".");
        let current: any = liveEditStore.dirtyProduct as any;
