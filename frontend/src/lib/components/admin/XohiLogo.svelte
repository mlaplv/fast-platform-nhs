...
>
  ...
  <div 
    class="absolute -inset-4 rounded-full border border-white/5 transition-all duration-700 {isHovered ? 'scale-110 opacity-100' : 'scale-100 opacity-60'}"
    style="box-shadow: 0 0 50px {activeTheme.aura}; border-color: {activeTheme.core}22"
  ></div>

  <!-- SVG QUANTUM MATRIX -->
  <svg 
    viewBox="-10 -10 120 120" 
    class="absolute inset-0 w-full h-full overflow-visible drop-shadow-[0_0_20px_{activeTheme.glow}]"
  >
    <defs>
