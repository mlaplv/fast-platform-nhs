<script lang="ts">
  ...
  const svgSize = $derived(
    typeof size === 'string' ? (sizeMap[size] || 80) :
    (size || sizeMap[variant] || 80)
  );
...
<div 
  class="relative flex items-center justify-center transition-all duration-700 {interactive || variant === 'header' ? 'cursor-pointer active:scale-95 group' : 'pointer-events-none'}"
