...
<div class="space-y-8 pb-20">
  ...
  <div class="flex items-center justify-between border-b border-white/5 pb-4">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h3 class="text-sm font-black text-white uppercase tracking-widest">QUẢN LÝ BỘ CÂU HỎI</h3>
        <p class="text-[8px] text-blue-400 font-mono tracking-widest mt-1">SOURCE: {path.toUpperCase()} // SYNC_ACTIVE</p>
      </div>
    </div>
    <button 
      onclick={addQuestion}
      class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all"
    >
    ...
    {#each questions as question, qIndex (qIndex)}
      ...
      >
        ...
        <div class="px-6 py-4 bg-white/[0.03] flex items-center justify-between border-b border-white/5">
          <div class="flex items-center gap-3">
            <span class="text-[10px] font-black text-blue-400/50 font-mono">#{qIndex + 1}</span>
            <input 
              bind:value={question.title}
              class="bg-transparent border-none text-white font-bold text-sm outline-none w-64 placeholder:text-white/10 focus:text-blue-400 transition-colors"
        ...
        <div class="p-6 space-y-4">
          <div class="flex items-center justify-between">
            <h4 class="text-[9px] font-black text-white/30 uppercase tracking-[0.2em] italic">Danh sách lựa chọn</h4>
            <button onclick={() => addOption(qIndex)} class="text-[9px] font-black text-blue-400/80 hover:text-blue-400 uppercase tracking-widest flex items-center gap-1 transition-all"><Plus size={12} /> THÊM OPTION</button>
          </div>
            ...
            {#each question.options as opt, optIndex (optIndex)}
              <div class="flex items-center gap-2 bg-white/[0.03] p-2 rounded-xl border border-white/5 group">
                ...
                <div class="flex-1 space-y-1">
                  <input bind:value={opt.label} class="w-full bg-transparent border-none text-[10px] font-bold text-white outline-none placeholder:text-white/5" placeholder="Tên lựa chọn" />
