...
<div class="w-full h-full flex flex-col md:flex-row gap-6 p-2 animate-in fade-in duration-700">
  ...
  <div class="flex-[1.5] bg-black/40 border border-white/10 rounded-3xl flex flex-col overflow-hidden relative shadow-2xl">
    <div class="h-14 border-b border-white/10 bg-white/5 flex items-center justify-between px-6 shrink-0">
      ...
      <div class="flex bg-black/50 p-1 rounded-xl border border-white/5 disabled-group">
         <button 
           class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all {previewMode === 'desktop' ? 'bg-white/20 text-white shadow-sm' : 'text-white/40 hover:text-white/80'}"
           onclick={() => previewMode = 'desktop'}
         >
           Desktop
         </button>
         <button 
           class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all {previewMode === 'mobile' ? 'bg-white/20 text-white shadow-sm' : 'text-white/40 hover:text-white/80'}"
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar flex items-start justify-center p-4 md:p-8 bg-[#f8f9fa] dark:bg-[#0c0a0f]">
       <!-- Adaptive Container -->
       <div class="transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] {previewMode === 'mobile' ? 'w-[375px] max-w-full min-h-[812px] bg-white text-black shadow-2xl rounded-[3rem] p-6 border-[8px] border-zinc-800' : 'w-full max-w-3xl bg-white text-black shadow-xl rounded-2xl p-8 md:p-12'}">
  ...
  <div class="flex-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar pb-10">
    ...
    <!-- 1. TRUST BADGES -->
    <div class="bg-gradient-to-br from-indigo-900/40 to-purple-900/40 border border-purple-500/30 rounded-3xl p-6 relative overflow-hidden group shrink-0">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="bg-black/50 border border-emerald-500/30 rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-1 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
        ...
        <div class="bg-black/50 border border-blue-500/30 rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-1 shadow-[0_0_15px_rgba(59,130,246,0.1)]">
           <span class="text-2xl">⚡</span>
        ...
        <div class="bg-black/50 border border-fuchsia-500/30 rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-1 shadow-[0_0_15px_rgba(217,70,239,0.1)]">
           <span class="text-2xl">🤖</span>
    ...
    <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 flex flex-col gap-3 shrink-0">
    ...
    <div class="bg-gradient-to-b from-[#e8f0fe] to-white rounded-3xl p-6 shadow-xl border border-blue-100 flex flex-col gap-3 relative overflow-hidden shrink-0">
       ...
       <div class="mt-2 flex gap-2">
          <div class="bg-white border border-gray-200 rounded-xl p-2 w-32 shadow-sm shrink-0 flex gap-2 items-center cursor-pointer hover:bg-gray-50">
    ...
    <div class="bg-white rounded-3xl outline outline-1 outline-gray-200 shadow-xl overflow-hidden flex flex-col font-sans shrink-0">
