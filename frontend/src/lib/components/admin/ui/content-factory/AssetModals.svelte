    ...
    >
      ...
      <div class="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-white/[0.02]">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-xl font-black text-white tracking-widest leading-none">THƯ VIỆN HỆ THỐNG</h2>
            <p class="text-[9px] text-white/30 font-bold tracking-[0.3em] mt-1.5 flex items-center gap-2">
      ...
      <div class="px-8 py-4 bg-black/40 border-t border-white/5 flex items-center justify-between">
        <div class="flex items-center gap-4 text-[7px] font-mono text-white/10 tracking-[0.3em]">
        ...
        <button
          onclick={onCloseLibrary}
          class="px-8 py-3 bg-white/5 hover:bg-white/10 text-white/40 hover:text-white rounded-xl text-[10px] font-black tracking-widest transition-all border border-white/10 active:scale-95"
        >
          Đóng thư viện
        </button>
      </div>
  ...
  >
    ...
    >
      <div class="flex flex-col items-center text-center gap-6">
        ...
        <div>
          <h3 class="text-lg font-black text-white tracking-widest leading-none">XÁCH NHẬN XOÁ VĨNH VIỄN?</h3>
          <p class="text-[10px] text-red-400 font-bold tracking-widest mt-2">Ảnh này sẽ bị xóa khỏi hệ thống, không thể hoàn tác!</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 w-full">
          <button
            ...
            class="py-3 px-4 bg-white/5 hover:bg-white/10 text-white/60 font-black text-[10px] tracking-widest rounded-xl transition-all border border-white/5"
          >
            Hủy bỏ
          </button>
          <button
            onclick={() => { if (pendingPurgeAsset) onConfirmPurge(pendingPurgeAsset); }}
            class="py-3 px-4 bg-red-600 hover:bg-red-500 text-white font-black text-[10px] tracking-widest rounded-xl transition-all shadow-[0_10px_30px_rgba(220,38,38,0.4)] active:scale-95"
          >
            XÁC NHẬN XOÁ
          </button>
        </div>
