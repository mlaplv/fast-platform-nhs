<script lang="ts">
  ...
  let activeDropdown = $state<"product" | "article" | "aspectRatio" | "duration" | "style" | null>(null);

  function toggleDropdown(name: "product" | "article" | "aspectRatio" | "duration" | "style", e: Event) {
    e.stopPropagation();
        ...
        <div>
          ...
          <div class="flex items-center justify-between pb-4 border-b border-gray-900 mb-6">
            <div class="flex items-center gap-2">
              ...
              <h3 class="text-xs font-bold text-white tracking-widest uppercase">
                {generatorStep === 1 ? 'BƯỚC 1: THIẾT LẬP NGUỒN' : 'BƯỚC 2: CHIẾN LƯỢC CẠNH TRANH'}
              </h3>
            </div>
            <button
          ...
          {#if isAnalyzing}
            ...
            <div class="py-20 flex flex-col items-center justify-center gap-6">
              ...
              <div class="text-center space-y-2">
                <p class="text-xs font-semibold text-cyan-400 animate-pulse">Quét Google Search & Phân tích đối thủ...</p>
                <p class="text-[9px] font-mono text-gray-500 leading-relaxed max-w-[280px] mx-auto">
                  Hệ thống đang thu thập chiến dịch của đối thủ trên thị trường và lập luận ưu thế sản phẩm của bạn.
                </p>
              </div>
            </div>
            ...
            <div class="py-20 flex flex-col items-center justify-center gap-6">
              ...
              <div class="w-full max-w-[280px] space-y-3 mt-4 mx-auto">
                <div class="flex items-center gap-3 text-xs">
                  <span class="w-4 h-4 rounded-full bg-cyan-950 text-cyan-400 border border-cyan-500 flex items-center justify-center text-[9px] font-mono">
                    ✓
                  </span>
            ...
            <div class="space-y-5">
              ...
              {#if sourceType === "product"}
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    >
                      <span class="truncate">{
                        !selectedProductId ? "Chọn sản phẩm..." :
                        products.find(p => p.id === selectedProductId)?.name || "Chọn sản phẩm..."
                      }</span>
                      <ChevronDown class="w-4 h-4 text-gray-505 transition-transform duration-200 {activeDropdown === 'product' ? 'rotate-180 text-cyan-400' : ''}" />
                    </button>
                      ...
                      >
                        {#if products.length === 0}
                          <button
                            type="button"
                            disabled
                            class="w-full text-left px-3 py-2 text-xs text-gray-500 cursor-not-allowed"
                          >
                        ...
                        {:else}
                          {#each products as prod}
                            <button
                              type="button"
                              onclick={() => { selectedProductId = prod.id; activeDropdown = null; }}
                              class="w-full text-left px-3 py-2 text-xs transition-colors truncate
                                     {selectedProductId === prod.id ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                            >
                  ...
                  <p class="text-[9px] font-mono text-gray-500 leading-normal">
                    Hệ thống sẽ chạy Google Search đối thủ và phân tích ưu điểm sản phẩm để làm định hướng kịch bản.
                  </p>
                </div>
              {:else if sourceType === "article"}
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    >
                      <span class="truncate">{
                        !selectedArticleId ? "Chọn bài viết..." :
                        articles.find(a => a.id === selectedArticleId)?.title || "Chọn bài viết..."
                      }</span>
                      <ChevronDown class="w-4 h-4 text-gray-505 transition-transform duration-200 {activeDropdown === 'article' ? 'rotate-180 text-cyan-400' : ''}" />
                    </button>
                      ...
                      >
                        ...
                        {:else}
                          {#each articles as art}
                            <button
                              type="button"
                              onclick={() => { selectedArticleId = art.id; activeDropdown = null; }}
                              class="w-full text-left px-3 py-2 text-xs transition-colors truncate
                                     {selectedArticleId === art.id ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                            >
                              {art.title}
                            </button>
                          {/each}
                  ...
                  <p class="text-[9px] font-mono text-gray-500 leading-normal">
                    Chuyển hóa nội dung bài viết tin tức/chia sẻ thành kịch bản phân cảnh video.
                  </p>
                </div>
              {:else}
                <div class="space-y-2">
                  ...
                  ></textarea>
                  <p class="text-[9px] font-mono text-gray-500 leading-normal">
                    Viết bất kỳ ý tưởng thô nào của bạn, AI sẽ xây dựng chiến lược và kịch bản.
                  </p>
                </div>
              {/if}
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    >
                      <span class="truncate">{
                        aspectRatio === "9:16" ? "Dọc (9:16) - TikTok/Reels" :
                        aspectRatio === "16:9" ? "Ngang (16:9) - YouTube/PC" :
                        "Vuông (1:1) - Instagram"
                      }</span>
                      <ChevronDown class="w-4 h-4 text-gray-505 transition-transform duration-200 {activeDropdown === 'aspectRatio' ? 'rotate-180 text-cyan-400' : ''}" />
                    </button>
                    ...
                    {#if activeDropdown === 'aspectRatio'}
                      ...
                      >
                        <button
                          type="button"
                          onclick={() => { aspectRatio = "9:16"; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {aspectRatio === '9:16' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          Dọc (9:16) - TikTok/Reels
                        </button>
                        <button
                          type="button"
                          onclick={() => { aspectRatio = "16:9"; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {aspectRatio === '16:9' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          Ngang (16:9) - YouTube/PC
                        </button>
                        <button
                          type="button"
                          onclick={() => { aspectRatio = "1:1"; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {aspectRatio === '1:1' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          Vuông (1:1) - Instagram
                        </button>
                      </div>
                ...
                <div class="space-y-2">
                  ...
                  <div class="relative">
                    ...
                    >
                      <span class="truncate">{
                        targetDuration === 15 ? "15 giây (Cực ngắn)" :
                        targetDuration === 30 ? "30 giây (Tiêu chuẩn)" :
                        targetDuration === 60 ? "60 giây (Chi tiết)" :
                        "90 giây (Kể chuyện)"
                      }</span>
                      <ChevronDown class="w-4 h-4 text-gray-505 transition-transform duration-200 {activeDropdown === 'duration' ? 'rotate-180 text-cyan-400' : ''}" />
                    </button>
                    ...
                    {#if activeDropdown === 'duration'}
                      ...
                      >
                        <button
                          type="button"
                          onclick={() => { targetDuration = 15; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {targetDuration === 15 ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          15 giây (Cực ngắn)
                        </button>
                        <button
                          type="button"
                          onclick={() => { targetDuration = 30; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {targetDuration === 30 ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          30 giây (Tiêu chuẩn)
                        </button>
                        <button
                          type="button"
                          onclick={() => { targetDuration = 60; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {targetDuration === 60 ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          60 giây (Chi tiết)
                        </button>
                        <button
                          type="button"
                          onclick={() => { targetDuration = 90; activeDropdown = null; }}
                          class="w-full text-left px-3 py-2 text-xs transition-colors
                                 {targetDuration === 90 ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                        >
                          90 giây (Kể chuyện)
                        </button>
                      </div>
              ...
              <div class="space-y-2">
                ...
                <div class="relative">
                  ...
                  >
                    <span class="truncate">{
                      !selectedStyleId ? "Chọn phong cách..." :
                      styles.find(s => s.id === selectedStyleId) ? `[${styles.find(s => s.id === selectedStyleId)!.platform}] ${styles.find(s => s.id === selectedStyleId)!.name}` : "Chọn phong cách..."
                    }</span>
                    <ChevronDown class="w-4 h-4 text-gray-505 transition-transform duration-200 {activeDropdown === 'style' ? 'rotate-180 text-cyan-400' : ''}" />
                  </button>
                    ...
                    >
                      ...
                      {:else}
                        {#each styles as style}
                          <button
                            type="button"
                            onclick={() => { selectedStyleId = style.id; activeDropdown = null; }}
                            class="w-full text-left px-3 py-2 text-xs transition-colors truncate
                                   {selectedStyleId === style.id ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                          >
              ...
              {#if styles.find(s => s.id === selectedStyleId)}
                ...
                <div class="bg-[#0c0c0c] border border-cyan-500/10 rounded-lg p-3 space-y-2">
                  <span class="text-[9px] font-mono text-cyan-400 font-bold uppercase tracking-widest block font-sans">CHI TIẾT PHONG CÁCH ĐÃ CHỌN</span>
          ...
          {:else if generatorStep === 2 && competitorAnalysis}
            ...
            <div class="space-y-5">
              <div class="bg-cyan-950/15 border border-cyan-500/10 rounded-lg p-3 space-y-1">
                <p class="text-xs font-semibold text-cyan-300">Đã thiết lập xong định vị chiến lược</p>
                <p class="text-[10px] text-gray-400 leading-relaxed">
                  Dưới đây là kết quả phân tích phản biện đối thủ cạnh tranh. Hãy tùy chỉnh theo đúng thực tế trước khi AI viết kịch bản.
                </p>
              </div>

              <!-- Competitor Weaknesses -->
              <div class="space-y-2">
                <span class="text-[9px] font-mono text-pink-500 tracking-wider font-semibold uppercase block">3 ĐIỂM YẾU CỦA ĐỐI THỦ CẠNH TRANH</span>
                {#each competitorAnalysis.competitor_weaknesses as weakness, idx}
                  <div class="flex gap-2 items-center">
                    <span class="text-[10px] font-mono text-pink-500/60 font-bold">#{idx + 1}</span>
                    <input
                      type="text"
              ...
              <!-- Our Strengths / USP -->
              <div class="space-y-2">
                <span class="text-[9px] font-mono text-cyan-400 tracking-wider font-semibold uppercase block">3 ĐIỂM MẠNH NỔI TRỘI CỦA TA (USP)</span>
                {#each competitorAnalysis.our_strengths as strength, idx}
                  <div class="flex gap-2 items-center">
                    <span class="text-[10px] font-mono text-cyan-400/60 font-bold">#{idx + 1}</span>
                    <input
                      type="text"
              ...
              <!-- Core Message -->
              <div class="space-y-2">
                <label for="core-message-input" class="text-[9px] font-mono text-yellow-500 tracking-wider font-semibold uppercase block">THÔNG ĐIỆP CỐT LÕI (CORE MESSAGE)</label>
                <textarea
