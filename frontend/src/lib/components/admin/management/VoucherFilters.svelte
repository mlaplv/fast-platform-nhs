...
<div class="px-6 py-4 border-b border-white/5 bg-[#080808] flex flex-wrap items-center justify-between gap-4">
  <div class="flex items-center gap-4 flex-1 min-w-[300px]">
    ...
    <div class="flex items-center bg-white/[0.03] border border-white/10 rounded-full p-1">
      <button
        ...
        class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase tracking-widest transition-all {activeFilter === 'all' ? 'bg-neon-cyan text-black font-bold' : 'text-gray-500 hover:text-white'}"
      >
        Tất cả
      </button>
      <button
        onclick={() => activeFilter = "active"}
        class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase tracking-widest transition-all {activeFilter === 'active' ? 'bg-neon-cyan text-black font-bold' : 'text-gray-500 hover:text-white'}"
      >
        Đang chạy
      </button>
      <button
  ...
  <div class="flex items-center gap-3">
    ...
    <button
      ...
      class="flex items-center gap-2 px-3 py-2 rounded-lg border border-white/10 text-[10px] font-mono uppercase tracking-widest hover:bg-white/5 transition-all {isAllSelected ? 'text-neon-cyan border-neon-cyan/40 bg-neon-cyan/5' : 'text-gray-400'}"
    >
    ...
    <button
      onclick={onAddNew}
      class="flex items-center gap-2 bg-neon-cyan text-black px-5 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all shadow-[0_0_20px_rgba(0,243,255,0.2)]"
    >
