<script lang="ts">
  ...
  // Presets State
  let selectedCameraStyle = $state<"none" | "cinematic" | "studio" | "3d_render">("none");
  let selectedLighting = $state<"none" | "volumetric" | "neon">("none");
  let arOverride = $state<"default" | "9:16" | "16:9" | "1:1">("default");
  let selectedCameraMovement = $state<"none" | "zoom_in" | "pan_left" | "dolly_zoom">("none");

  // Dropdown active state
  let activeDropdown = $state<"camera" | "lighting" | "ar" | "movement" | null>(null);

  function toggleDropdown(name: "camera" | "lighting" | "ar" | "movement", e: Event) {
    e.stopPropagation();
  ...
  function buildMidjourneyPrompt(scene: VideoScene, aspect: string) {
    ...
    if (selectedCameraStyle === "cinematic") {
      base += ", 35mm cinematic photograph, shallow depth of field, anamorphic lenses";
    } else if (selectedCameraStyle === "studio") {
      base += ", commercial studio photography, high-end clean lighting, crisp product details";
    } else if (selectedCameraStyle === "3d_render") {
      base += ", octane 3D product render, raytracing, unreal engine 5 style, hyper-detailed";
    }
...
{#if showPromptHub}
  ...
  >
      ...
      {#if activePromptTab === 'midjourney'}
        <div class="px-5 py-3.5 bg-[#0b0c10]/40 border-b border-gray-900/60 grid grid-cols-3 gap-3" transition:slide>
          <!-- Camera Style Preset -->
          <div class="space-y-1.5">
            <span class="flex items-center gap-1 text-[9px] font-mono text-gray-500 uppercase font-bold">
            ...
            <div class="relative">
              ...
              >
                <span class="truncate">{
                  selectedCameraStyle === "none" ? "Mặc định kịch bản" :
                  selectedCameraStyle === "cinematic" ? "Cinematic 35mm" :
                  selectedCameraStyle === "studio" ? "Studio Commercial" :
                  "3D Octane Render"
                }</span>
                <ChevronDown class="w-3 h-3 text-gray-500 transition-transform duration-200 {activeDropdown === 'camera' ? 'rotate-180 text-cyan-400' : ''}" />
              </button>
              ...
              {#if activeDropdown === 'camera'}
                ...
                >
                  <button
                    type="button"
                    onclick={() => { selectedCameraStyle = "none"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraStyle === 'none' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Mặc định kịch bản
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraStyle = "cinematic"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraStyle === 'cinematic' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Cinematic 35mm
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraStyle = "studio"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraStyle === 'studio' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Studio Commercial
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraStyle = "3d_render"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraStyle === '3d_render' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    3D Octane Render
                  </button>
                </div>
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              >
                <span class="truncate">{
                  selectedLighting === "none" ? "Mặc định kịch bản" :
                  selectedLighting === "volumetric" ? "Volumetric (Luồng)" :
                  "Neon Cyberpunk"
                }</span>
                <ChevronDown class="w-3 h-3 text-gray-500 transition-transform duration-200 {activeDropdown === 'lighting' ? 'rotate-180 text-cyan-400' : ''}" />
              </button>
              ...
              {#if activeDropdown === 'lighting'}
                ...
                >
                  <button
                    type="button"
                    onclick={() => { selectedLighting = "none"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedLighting === 'none' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Mặc định kịch bản
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedLighting = "volumetric"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedLighting === 'volumetric' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Volumetric (Luồng sáng)
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedLighting = "neon"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedLighting === 'neon' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Neon Cyberpunk
                  </button>
                </div>
          ...
          <div class="space-y-1.5">
            ...
            <div class="relative">
              ...
              >
                <span class="truncate">{
                  arOverride === "default" ? "Dùng của kịch bản" :
                  arOverride === "9:16" ? "Dọc (9:16)" :
                  arOverride === "16:9" ? "Ngang (16:9)" :
                  "Vuông (1:1)"
                }</span>
                <ChevronDown class="w-3 h-3 text-gray-500 transition-transform duration-200 {activeDropdown === 'ar' ? 'rotate-180 text-cyan-400' : ''}" />
              </button>
              ...
              {#if activeDropdown === 'ar'}
                ...
                >
                  <button
                    type="button"
                    onclick={() => { arOverride = "default"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {arOverride === 'default' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Dùng của kịch bản
                  </button>
                  <button
                    type="button"
                    onclick={() => { arOverride = "9:16"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {arOverride === '9:16' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Dọc (9:16) - Shorts/TikTok
                  </button>
                  <button
                    type="button"
                    onclick={() => { arOverride = "16:9"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {arOverride === '16:9' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Ngang (16:9) - Youtube
                  </button>
                  <button
                    type="button"
                    onclick={() => { arOverride = "1:1"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {arOverride === '1:1' ? 'bg-cyan-950/45 text-cyan-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Vuông (1:1)
                  </button>
                </div>
      ...
      {#if activePromptTab === 'runway'}
        <div class="px-5 py-3.5 bg-[#0b0c10]/40 border-b border-gray-900/60" transition:slide>
          ...
          <div class="space-y-1.5 max-w-xs">
            ...
            <div class="relative">
              ...
              >
                <span class="truncate">{
                  selectedCameraMovement === "none" ? "Mặc định kịch bản" :
                  selectedCameraMovement === "zoom_in" ? "Slow Zoom In" :
                  selectedCameraMovement === "pan_left" ? "Slow Pan Left" :
                  "Dolly Zoom Vertigo"
                }</span>
                <ChevronDown class="w-3 h-3 text-gray-500 transition-transform duration-200 {activeDropdown === 'movement' ? 'rotate-180 text-purple-400' : ''}" />
              </button>
              ...
              {#if activeDropdown === 'movement'}
                ...
                >
                  <button
                    type="button"
                    onclick={() => { selectedCameraMovement = "none"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraMovement === 'none' ? 'bg-purple-950/45 text-purple-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Mặc định kịch bản
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraMovement = "zoom_in"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraMovement === 'zoom_in' ? 'bg-purple-950/45 text-purple-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Slow Zoom In (Cận cảnh chậm)
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraMovement = "pan_left"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraMovement === 'pan_left' ? 'bg-purple-950/45 text-purple-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
                    Slow Pan Left (Lia máy trái)
                  </button>
                  <button
                    type="button"
                    onclick={() => { selectedCameraMovement = "dolly_zoom"; activeDropdown = null; }}
                    class="w-full text-left px-3 py-1.5 text-[10px] transition-colors
                           {selectedCameraMovement === 'dolly_zoom' ? 'bg-purple-950/45 text-purple-400 font-semibold' : 'text-gray-400 hover:bg-white/5 hover:text-white'}"
                  >
      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
        {#if activeScript && activeScript.structured_script?.scenes}
          ...
          {#each scenes as scene, idx}
            {@const aspect = activeScript.structured_script.aspect_ratio || "16:9"}
            {@const currentAspect = arOverride === "default" ? aspect : arOverride}
            {@const promptText = activePromptTab === 'midjourney' 
              ? buildMidjourneyPrompt(scene, aspect)
              : activePromptTab === 'runway' 
                ? buildRunwayPrompt(scene)
            ...
            <div class="bg-[#0b0b0f] border border-gray-900 rounded-xl p-4 space-y-3 relative group">
              <div class="flex items-center justify-between border-b border-gray-900/50 pb-2">
                ...
                <button 
                  onclick={() => copyPromptToClipboard(promptText, scene.scene_number || (idx + 1))}
              ...
              <div class="flex items-center justify-between text-[9px] text-gray-500 font-mono">
                <span>Tỷ lệ: {activePromptTab === 'midjourney' ? currentAspect : aspect}</span>
