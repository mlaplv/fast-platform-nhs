...
<div
  class="group relative z-10 hover:z-20 flex flex-col md:grid md:grid-cols-[60px_60px_minmax(250px,2fr)_1fr_120px] md:items-center bg-[#0a0a0a] md:bg-transparent border border-white/5 md:border-none rounded-xl md:rounded-none p-4 sm:p-0 hover:bg-white/[0.04] transition-[background-color,border-color] duration-150 {isExpanded
...
>
  ...
  <div class="hidden md:flex items-center justify-center p-4 border-r border-white/[0.01]">
    <button 
      ...
      class="w-4 h-4 rounded border {isSelected ? 'bg-[#00FFFF] border-[#00FFFF]' : 'border-white/10 bg-black/40'} flex items-center justify-center transition-[background-color,border-color] hover:border-[#00FFFF]/40 shadow-inner"
    >
      {#if isSelected}
  ...
  <div class="hidden md:flex items-center justify-center p-4 border-r border-white/[0.01]">
    <button
      ...
      class="p-2 bg-black/40 hover:bg-[#00FFFF]/10 border border-transparent hover:border-[#00FFFF]/20 shadow-sm rounded-xl transition-transform duration-200 {isExpanded
  ...
  <div class="sm:px-6 sm:py-5 relative z-10">
    <div class="flex items-start sm:items-center gap-3">
      ...
      <div class="md:hidden">
        <button
          onclick={() => onToggleExpand(user.id)}
          class="p-2 bg-black/40 hover:bg-[#00FFFF]/10 border border-transparent hover:border-[#00FFFF]/20 shadow-sm rounded-xl transition-transform {isExpanded
            ? 'rotate-180 text-[#00FFFF] border-[#00FFFF]/20 bg-[#00FFFF]/5'
            : 'text-gray-500 hover:text-[#00FFFF]'}"
        >
          <ChevronDown size={14} />
  ...
  <div
    class="absolute top-4 right-4 sm:relative sm:top-0 sm:right-0 sm:px-6 sm:py-5 md:text-right relative z-30 sm:w-full"
  >
    <div
    ...
    >
      <button
        class="p-2 text-gray-500 hover:text-white transition-colors bg-white/5 sm:bg-transparent hover:bg-white/10 rounded-xl border border-white/5"
      >
        <MoreVertical size={16} />
      </button>
      
      <!-- Optimized Menu: Backdrop-blur disabled when invisible to save paint cost -->
      <div
        class="absolute right-0 top-full sm:pt-2 w-56 opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-[opacity,transform] duration-200 z-[var(--z-admin-hud)] origin-top-right group-hover/menu:scale-100 scale-95 mt-1 sm:mt-0 border border-white/10 rounded-xl bg-[#0a0a0a]/98 group-hover/menu:backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)]"
      >
...
...
  ...
  {#if isExpanded}
    <div
      class="col-span-1 md:col-span-12 mt-4 sm:mt-0 px-2 sm:px-0"
      transition:slide
    >
