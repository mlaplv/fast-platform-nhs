  ...
  <div class="container mx-auto px-6 max-w-7xl relative z-10">
    ...
    <header class="mb-32 md:mb-48 text-left md:flex items-end justify-between border-b border-white/5 pb-10">
      <div class="max-w-2xl">
        ...
        <h3 class="text-6xl md:text-8xl font-black text-white tracking-tighter leading-[0.9] uppercase italic mb-8">
          KHOẢNH KHẮC <br/> <span class="text-luxury-gold">CHIẾN THẮNG</span>
        </h3>
      </div>
          ...
          {#each timeline as item, i}
             <div class="story-node" class:active={visibleLevels > i}>
                <div class="node-indicator">
                   ...
                   <span class="node-day">0{item.day}</span>
                </div>
                <div class="node-content mt-12">
                   <h4 class="text-2xl font-black text-white italic mb-4 tracking-tighter">{item.title}</h4>
                   <p class="text-sm text-slate-400 font-medium leading-relaxed">{item.description}</p>
                </div>
             </div>
          {/each}
       </div>
    ...
    <div class="cinematic-bento pt-20 border-t border-white/5">
       <div class="section-title-hud mb-20 text-center md:text-left">
          <span class="text-[10px] font-black text-luxury-gold uppercase tracking-[0.6em] block mb-2">The_Winning_Output</span>
          <h4 class="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter">SỐNG TRỌN <span class="text-luxury-sakura">TỪNG GIÂY PHÚT</span></h4>
       </div>
             ...
             {#if visibleLevels > 3}
                <div class="scene-card p-12 md:p-16 h-full flex flex-col justify-end relative overflow-hidden group" in:fly={{ y: 50, duration: 1200 }}>
                   ...
                   <div class="relative z-10">
                      <span class="text-[9px] font-black text-luxury-gold uppercase tracking-[0.4em] mb-3 block">{scenarios[0].title}</span>
                      <h5 class="text-3xl md:text-5xl font-black text-white italic mb-4 leading-none">{scenarios[0].result}</h5>
                      <p class="text-sm md:text-lg text-slate-300 font-medium max-w-md opacity-0 group-hover:opacity-100 transition-all duration-700 transform translate-y-4 group-hover:translate-y-0">{scenarios[0].desc}</p>
                   </div>
                </div>
             {/if}
          </div>
                ...
                <div class="bento-item sub-scene" style:--parallax="{scrollY * scene.parallax * 0.005}px">
                   {#if visibleLevels > (i + 4)}
                      <div class="scene-card p-8 md:p-10 h-full flex flex-col justify-end group" in:fly={{ x: 30, duration: 1000, delay: i * 200 }}>
                         ...
                         <div class="relative z-10">
                            <span class="text-[8px] font-black text-white/40 uppercase tracking-widest mb-1 block">{scene.title}</span>
                            <h5 class="text-xl md:text-2xl font-bold text-white italic leading-tight">{scene.result}</h5>
                            <p class="text-xs text-slate-400 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">{scene.desc}</p>
                         </div>
                      </div>
                   {/if}
                </div>
          ...
          <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
             <div class="text-left">
                <div class="flex items-center gap-3 mb-6">
                   ...
                   <span class="text-luxury-gold text-xs font-black uppercase tracking-[0.4em]">Limited_Allocation</span>
                </div>
                <h6 class="text-4xl md:text-6xl font-black text-white italic tracking-tighter leading-none mb-6">
                   CHỈ CÒN <span class="text-luxury-sakura transition-all duration-1000 tabular-nums">{displaySlots < 10 ? '0' + displaySlots : displaySlots} SUẤT</span> <br/> ƯU ĐÃI TẶNG ẨN
                </h6>
             </div>
             
             <div class="hud-metrics flex gap-8 md:gap-16">
                <div class="metric-box">
                   <div class="text-white/20 text-[8px] font-black uppercase tracking-widest mb-2">Inventory_Status</div>
                   <div class="text-white text-xl font-bold italic">SẮP CHÁY HÀNG</div>
                   <div class="w-full h-px bg-red-500/30 mt-2"></div>
                </div>
                <div class="metric-box">
                   <div class="text-white/20 text-[8px] font-black uppercase tracking-widest mb-2">Demand_Index</div>
                   <div class="text-luxury-gold text-xl font-bold italic font-mono uppercase">Extreme</div>
