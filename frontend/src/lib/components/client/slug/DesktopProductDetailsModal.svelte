...
<div use:portal class="desktop-product-details-modal fixed inset-0 flex items-center justify-center p-6 md:p-12" style:z-index={Z_INDEX_CLIENT.MODAL}>
  ...
  >
    ...
    <div class="px-10 pt-10 pb-6 border-b border-white/5 bg-[#0a0a0a] shrink-0">
      <h2 class="text-[14px] font-black uppercase tracking-[0.3em] italic text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-400 flex items-center gap-3">
    ...
    <div class="px-10 py-8 overflow-y-auto custom-scrollbar flex-1 elite-prose">
      ...
      {:else}
        <div class="flex flex-col items-center justify-center h-full min-h-[300px] text-white/30 space-y-4">
          ...
          <p class="text-[11px] uppercase tracking-[0.4em] font-bold">Chưa có thông tin mô tả chi tiết</p>
        </div>
      {/if}
    </div>
    ...
    <div class="px-10 py-6 border-t border-white/5 bg-[#0a0a0a] flex items-center justify-between">
      <div class="flex items-center gap-3 text-[10px] font-bold text-white/30 uppercase tracking-[0.3em] italic">
      ...
      <button 
        onclick={close}
        class="px-8 py-2.5 bg-white/5 hover:bg-white/10 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-full border border-white/10 transition-all font-bold"
      >
        Đóng lại
      </button>
    </div>
