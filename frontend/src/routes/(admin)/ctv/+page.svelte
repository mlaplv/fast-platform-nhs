<script lang="ts">
  ...
  async function exportAllCommissionsToExcel() {
    ui.showToast('Đang tải dữ liệu đối soát toàn hệ thống...', 'info');
    try {
      const res = await apiClient.get<any>('/admin/ctv/commissions?page=1&page_size=5000');
      const items = res.items || [];
      if (items.length === 0) {
        ui.showToast('Không có dữ liệu đối soát để xuất', 'warning');
        return;
      }
      
      exportLedgerToExcel(items, 'DoiSoat_ToanHeThong_Osmo');
      ui.showToast('Đã xuất file đối soát hoa hồng toàn hệ thống thành công!', 'success');
    } catch (e: any) {
      ui.showToast(e.message || 'Lỗi tải dữ liệu xuất Excel', 'error');
    }
  }
      ...
      <div class="space-y-6" in:fade>
        ...
        <div class="flex flex-col md:flex-row gap-4 bg-stone-950/80 border border-stone-800/40 p-4 rounded-xl backdrop-blur-md">
          ...
          <div class="flex gap-4">
            ...
            <button
              onclick={exportAllCommissionsToExcel}
