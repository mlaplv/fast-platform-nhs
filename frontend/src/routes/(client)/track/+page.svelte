<script lang="ts">
  ...
  async function handleTrack() {
    if (!orderId || !phone) {
        showToast("Vui lòng điền đủ Mã đơn và Số điện thoại thưa sếp!", "error");
        return;
    }

    isSubmitting = true;
    try {
        // Try to fetch order with phone verification thưa sếp!
        ...
        if (res) {
            ...
            setTimeout(() => goto(`/checkout/success/${orderId.trim()}?phone=${cleanPhone}`), 1000);
        }
    } catch (e: any) {
        showToast(e.message || "Không tìm thấy đơn hàng hoặc thông tin không khớp thưa sếp!", "error");
    } finally {
        isSubmitting = false;
    }
  }
...
<div class="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
  ...
  >
    ...
    <div class="space-y-6">
      ...
      <div class="space-y-2">
        ...
        <input 
          type="tel" 
          bind:value={phone}
          placeholder="Nhập SĐT khi đặt hàng"
    ...
    <p class="mt-10 text-[9px] text-slate-600 text-center font-bold uppercase tracking-widest leading-relaxed">
      Quý khách vui lòng nhập đúng thông tin để bảo mật dữ liệu đơn hàng thưa sếp!
    </p>
  </div>
