    ...
    <div class="pt-16 pb-20 px-4 space-y-6">
        <div class="space-y-6">
          {#if isLoading}
            <div class="py-20 flex flex-col items-center justify-center space-y-4">
              <div class="w-8 h-8 border-2 border-luxury-copper border-t-transparent animate-spin rounded-full"></div>
              <p class="text-[11px] text-stone-400 uppercase tracking-widest animate-pulse">Đang tải đơn hàng...</p>
            </div>
          {:else if orders.length === 0}
            <div class="py-24 text-center">
              <p class="text-stone-400 font-serif italic">Bạn chưa có đơn hàng nào.</p>
            </div>
          {:else}
            {#each orders as order (order.id)}
              {@const status = getStatusStyle(order.status)}
              <div class="bg-white p-4 border border-stone-100">
                <div class="flex justify-between items-center mb-4">
                  <span class="text-[11px] font-bold text-stone-500">#{order.id.slice(-8).toUpperCase()}</span>
                  <span class="text-[11px] font-black uppercase {status.color}">{status.label}</span>
                </div>
                {#each order.items as item}
                  <div class="flex gap-4 mb-2">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-stone-500">x{item.quantity}</p>
                    </div>
                  </div>
                {/each}
                <div class="mt-4 pt-4 border-t border-stone-100 text-right font-bold">
