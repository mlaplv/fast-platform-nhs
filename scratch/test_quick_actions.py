...
# Override container hostnames with localhost for host execution
...
async def main():
    ...
    async with session_maker() as db:
        slug_stmt = select(ProductBase.slug).where(ProductBase.status == "ACTIVE").limit(1)
        real_slug = await db.scalar(slug_stmt)
