...
### GIAO THỨC THIẾT QUÂN LUẬT V62.2 (VIRAL AGENTIC STANDARD)
...
1. **Phase 1: Classify (Nhận diện)**:
   ...
   - **T2 Cloud Router**: > Gemini 3.x (PydanticAI Agent).
...
**STT (Speech-To-Text) — Groq Whisper v3-turbo (V62.2 Optimizations):**

- **STT Anchors**: Tự động chèn từ khóa chuyên ngành vào `prompt` của Whisper (qua `VoiceProfile` cache ở Redis) để bắt dính giọng vùng miền.
- **Kill-Switch (Zero-Hallucination)**: Soi data `verbose_json` của Whisper, TỰ ĐỘNG hủy kết quả (trả rỗng) nếu `compression_ratio > 2.4` HOẶC `no_speech_prob > mic_sensitivity`.
- Loại bỏ hoàn toàn: regex band-aid, `stt_corrections.json`, `stop_words.json`.
