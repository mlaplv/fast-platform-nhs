...
- [x] **Tối ưu hóa hiệu năng vượt bậc (Lighthouse 90+):**
  ...
  - **Cấu trúc lại Router với derived loaderPromise và {#await}:** Thay thế cơ chế dynamic import chạy trễ trong `$effect` (vốn bắt buộc browser phải render và paint skeleton trước, sau đó mới paint lại component chính gây nhấp nháy màn hình và tăng TBT) bằng derived promise chạy ngay lập tức khi hydration kết hợp khối `{#await loaderPromise}`, tối ưu hóa tối đa Speed Index và Total Blocking Time. (Done)
