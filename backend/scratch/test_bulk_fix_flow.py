...
<p>Cập ẩm sâu và nuôi dưỡng da tay thường xuyên?</p>
...
<p><strong>Chăm sóc đôi bàn tay là một hành trình dài và cần sự kiên trì. With những bước nhỏ được thực hiện đều đặn mỗi ngày, bạn hoàn toàn có thể ngăn ngừa và cải thiện tình trạng lão hóa sớm, giữ cho đôi tay luôn là niềm tự hào của mình. Bạn đã sẵn sàng lắng nghe và yêu thương đôi tay mình nhiều hơn mỗi ngày chưa?</strong></p>"""

async def run():
    print("----------------------------------------")
    print("STEP 1: Analyzing initial draft content...")
    analyzer = SeoAnalyzer()
    campaign = AdHocContent(
        content=DRAFT_HTML,
        topic="Dấu dấu hiệu lão hóa da tay sớm và cách khắc phục hiệu quả",
        category="CREATIVE_CONTENT"
    )
    
    result = await analyzer.analyze(campaign, force=True)
    print(f"Initial Score: {result.total_score} | Grade: {result.grade}")
    
    # Check what annotations we have
    seo_annots = [ann.model_dump() for ann in result.seo_annotations]
    heading_annots = [a for a in seo_annots if a.get('type') in {"missing_headings", "fake_heading", "missing_h2"}]
    print(f"Heading annotations found: {len(heading_annots)}")
    for a in heading_annots[:5]:
        print(f"  - [{a['type']}] Text: {a['text']}")
        
    print("\n----------------------------------------")
    print("STEP 2: Executing Bulk Fix...")
    fix_req = BulkFixRequest(category="seo", annotations=seo_annots)
    fix_res = await analyzer.bulk_fix(campaign, fix_req)
    
    print("Bulk Fix Completed!")
    print(f"Logs: {len(fix_res.logs)} entries")
    for log in fix_res.logs:
        print(f"  {log}")
        
    new_content = fix_res.new_content
    # Let's count how many <h2> and <h3> we have now
    h2_count = len(re.findall(r'<h2[^>]*>', new_content, re.IGNORECASE))
    h3_count = len(re.findall(r'<h3[^>]*>', new_content, re.IGNORECASE))
    print(f"\nFixed content has: {h2_count}x <h2>, {h3_count}x <h3>")
    
    # Print the first few heading lines in the fixed draft
    headings = re.findall(r'<h[23][^>]*>.*?</h[23]>', new_content, re.IGNORECASE)
    print("First 5 converted headings in draft:")
    for h in headings[:5]:
        print(f"  {h}")

    print("\n----------------------------------------")
    print("STEP 3: Re-analyzing fixed content...")
    campaign.draft_content = new_content
    result2 = await analyzer.analyze(campaign, force=True)
    print(f"Final Score: {result2.total_score} | Grade: {result2.grade}")
