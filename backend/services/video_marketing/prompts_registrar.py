...
6. BẮT BUỘC ĐỐI CHIẾU THÔNG SỐ BAO BÌ VẬT LÝ DỰA TRÊN THÔNG TIN SẢN PHẨM:
   - Bạn PHẢI đối chiếu kỹ lưỡng với thông tin mô tả chi tiết và các thuộc tính trong [THÔNG IN NGUỒN PHÂN TÍCH] để xác định cấu trúc bao bì thực tế của sản phẩm:
     * Dạng bao bì (bao_bi): dạng tuýp bóp (squeeze tube), chai vòi nhấn (pump bottle), hũ (jar), hay lọ nhỏ giọt (dropper bottle).
     * Loại nắp & vị trí nắp (nap): nắp vặn (screw cap) hay nắp bật (snap-cap), nắp ở đỉnh đầu (top) hay nắp ở đáy (bottom).
     * Đầu lấy sản phẩm (dau_lay_san_pham): đầu vòi nhọn (pointed nozzle tip), đầu vòi nhấn (pump nozzle), hay ống hút nhỏ giọt (dropper pipette).
     * Thể chất & màu sắc (the_chat_san_pham): gel màu hồng nhạt, kem trắng mịn, tinh chất lỏng trong suốt, v.v.
   - Tuyệt đối CẤM tự ý bịa đặt các hành động vật lý hoặc bộ phận mâu thuẫn với cấu trúc thực tế (ví dụ: cấm mô tả "nhỏ giọt từ ống hút dropper" hoặc "xoay nắp thủy tinh ở đỉnh đầu" nếu sản phẩm thực tế là dạng TUÝP NHỰA có NẮP VẶN Ở ĐÁY và ĐẦU VÒI NHỌN).
   - Hành động mở và lấy sản phẩm trong visual_description phải khớp hoàn toàn với logic vật lý của bao bì đó (ví dụ: "xoay nắp vặn hình trụ dẹt ở đáy tuýp để lộ đầu vòi nhọn", "bóp nhẹ thân tuýp để gel serum màu hồng nhạt chảy ra từ đầu vòi nhọn").
   - Hãy viết các prompt AI chi tiết bám sát chính xác các thông tin vật lý này.
...
1. SỬA LỖI PROMPT AI (ai_generation_viability) — ƯU TIÊN SỐ 1:
   ...
   - BẮT BUỘC ĐỐI CHIẾU THÔNG SỐ BAO BÌ VẬT LÝ: Đối chiếu kỹ lưỡng với các đặc tính sản phẩm (bao bì, nắp, đầu lấy sản phẩm, thể chất) được cung cấp để sửa đổi các hành động mô tả sai dạng bao bì (ví dụ: sửa động tác "xoay nắp chai thủy tinh" hoặc "nhỏ giọt dropper" thành động tác mở nắp và lấy sản phẩm khớp với mô tả của sản phẩm thật, chẳng hạn như "bóp tuýp kem" hoặc "xoay mở nắp vặn hình trụ dẹt màu trắng ở đáy tuýp để lộ đầu vòi nhọn").
