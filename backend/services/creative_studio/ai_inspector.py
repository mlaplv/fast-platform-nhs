...
from backend.schemas.campaign import AiReadyReport, AutoFixResponse, BulkFixResponse, BulkFixRequest, AgentResponse, AgentSignal
...
class AiInspector:
    ...
    async def analyze(self, campaign: Dict[str, object], session: AsyncSession) -> AiReadyReport:
        """
        Performs full GEO analysis on draft content using Trinity Bridge.
        """
        draft = cast(str, campaign.get("draft_content") or "")
    ...
    async def auto_fix(self, campaign: Dict[str, object], req: AutoFixRequest, session: AsyncSession) -> AutoFixResponse:
        """
...
"""
    ...
    async def bulk_fix(self, campaign: Dict[str, object], req: BulkFixRequest, session: AsyncSession) -> BulkFixResponse:
        """
