...
class SeoAnalyzer:
    ...
    async def analyze(self, campaign: Dict[str, object], session: AsyncSession) -> SeoReport:
        """
        Analyzes draft content for SEO quality vs competitors.
        """
        draft: str = cast(str, campaign.get("draft_content") or "")

        # Phase 77: Using Standard Golden Thread Helpers
        primary: str = campaign_service.get_gold_val(campaign, "primary_keyword", "")
        secondary: List[str] = cast(List[str], campaign_service.get_gold_val(campaign, "secondary_keywords", []))
        title: str = campaign_service.get_gold_val(campaign, "title", "")
        persona: str = campaign_service.get_gold_val(campaign, "persona", "")

        outline: Dict[str, object] = cast(Dict[str, object], campaign.get("outline_data") or {})
