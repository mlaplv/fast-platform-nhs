...
from backend.schemas.campaign import ArticleOutline, AgentResponse, AgentSignal
...
class CreativePen:
    ...
    async def generate_outline(self, campaign: Dict[str, object]) -> ArticleOutline:
        """
    ...
    async def write_draft(self, campaign: Dict[str, object]) -> str:
        """
    ...
    async def stream_draft(self, campaign: Dict[str, object]):
        """
    ...
    async def _build_draft_prompt(self, campaign: Dict[str, object]):
        """Helper to build prompt (Shared between sync and stream)."""
        outline_data = campaign.get("outline_data") or {}
        assets = cast(List[Union[str, Dict[str, object]]], campaign.get("assets_data") or [])
        ...
        asset_lines = []
        for i, asset in enumerate(assets[:12], 1):
            url = asset if isinstance(asset, str) else asset.get("file_path") or asset.get("url")
            if url:
        ...
        if assets:
            ...
            first_asset_url = first_asset if isinstance(first_asset, str) else cast(str, first_asset.get("file_path") or first_asset.get("url") or "")

        avatar_url = campaign_service.get_gold_val(campaign, "avatar", first_asset_url or None)
        ...
        config = campaign_service.get_gold_config(campaign)
        # Memory Shield: Clamp word count to prevent OOM/hallucinations on 2GB VPS
