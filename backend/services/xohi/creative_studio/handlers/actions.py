...
class ActionHandler:
    ...
    async def approve_step(self, campaign_id: str, data: Dict[str, object], campaign_repo: ContentCampaignRepository) -> GenericResponse:
        ...
        if not data.get("approved", True):
            c.status = "REJECTED"; await campaign_repo.update(c); return GenericResponse(status="success", message="Đã ghi nhận sếp rejection.", data={"campaign_id": campaign_id})
    ...
    async def retry_step(self, campaign_id: str, campaign_repo: ContentCampaignRepository) -> GenericResponse:
        c = await campaign_repo.get(campaign_id)
        if not c: return GenericResponse(status="error", message="Campaign not found")
        c.status = "PROCESSING"; await campaign_repo.update(c)
        if hasattr(campaign_repo, "session"): await campaign_repo.session.commit()
        await event_bus.emit("CONTENT_PROGRESS", {"campaign_id": campaign_id, "user_id": c.user_id, "step": c.current_step, "message": "🔄 Rerunning...", "status": "PROCESSING", "timestamp": str(asyncio.get_event_loop().time())})
        asyncio.create_task(self.orchestrator._trigger_next_step(campaign_id, force_step=c.current_step))
        return GenericResponse(status="success", message="Đang chạy lại...")
    ...
    async def update_metadata(self, campaign_id: str, data: Dict[str, object], campaign_repo: ContentCampaignRepository) -> GenericResponse:
        ...
        for f in ["assets", "keywords", "outline_data", "draft_content", "final_html", "reserve_assets", "avatar", "selected_index"]:
            val = data.get(f)
            if val is not None:
                from sqlalchemy.orm.attributes import flag_modified
                if f == "assets": c.assets_data = val; flag_modified(c, "assets_data")
                elif f == "keywords": c.topic_data = val; flag_modified(c, "topic_data")
                elif f == "outline_data": c.outline_data = val; flag_modified(c, "outline_data")
                elif f in ["reserve_assets", "avatar", "selected_index"]: gold[f] = val; gold_changed = True
                elif f == "draft_content": c.draft_content = val
                elif f == "final_html": c.final_html = val
        if data.get("force_cache_invalidate"): gold["analysis_cache"], gold["analysis_metrics"], gold_changed = {}, {}, True
        if gold_changed: c.gold_metadata = gold; from sqlalchemy.orm.attributes import flag_modified; flag_modified(c, "gold_metadata")
