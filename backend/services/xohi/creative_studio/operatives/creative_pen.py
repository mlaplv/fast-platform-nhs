...
class CreativePen:
    ...
    async def generate_outline(self, campaign: ContentCampaign) -> ArticleOutline:
        ...
        config = campaign.get_gold_val("creation_config", {})
...
- **ĐỘ DÀI BẮT BUỘC**: Bài viết PHẢI nằm trong khoảng từ {min_words} đến {max_words} từ. Đây là yêu cầu tối thượng.
...
"""
        ...
        try:
            ...
            result = await trinity_bridge.run(self.draft_agent, prompt)
...
<section class="cta">
  ...
  <p>Hy vọng bài viết này giúp bạn hiểu rõ hơn về <strong>{primary}</strong>. Hãy để lại bình luận hoặc liên hệ với chúng tôi để được tư vấn thêm!</p>
    ...
    def _replace_image_placeholders(self, content: str, assets: list, alt_text: str = "") -> str:
        """
        V70.0: Guaranteed [IMAGE_N] replacement pass.
        Replaces all [IMAGE_N] tokens in content with proper <figure> HTML tags.
        """
        import re
        for i, url in enumerate(assets[:20], 1):
            ...
            if placeholder in content:
                ...
                logger.debug(f"[Content Factory] Replaced {placeholder}")
        ...
        if remaining:
            logger.warning(f"[Content Factory] Emptied {len(remaining)} unreplaced placeholders: {remaining}")
