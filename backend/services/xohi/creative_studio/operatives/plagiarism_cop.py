...
class PlagiarismCop:
    ...
    async def bulk_fix(self, campaign: ContentCampaign, req: BulkFixRequest) -> BulkFixResponse:
        ...
        """
        logs = []
        logs.append("🔍 Khởi động hệ thống phẫu thuật nội dung...")
        ...
        for para in paragraphs:
            ...
            if not is_dup:
                ...
                msg = f"✂️ Đã cắt bỏ đoạn văn trùng lặp nội bộ: \"{para[:40]}...\""
                logger.info(f"[PlagiarismCop] bulk_fix: {msg}")
                logs.append(msg)
        ...
        # Split into paragraphs first, then sentences, to preserve structure.
        ...
        for para, _ in kept:
            ...
            for s in para_sentences:
                ...
                if norm not in seen_norms:
                    ...
                    dedup_count += 1
                    msg = f"✂️ Đã loại bỏ câu lặp nội bộ: \"{s[:40]}...\""
                    logger.info(f"[PlagiarismCop] bulk_fix: {msg}")
                    logs.append(msg)
            
            if para_kept:
                final_paragraphs.append(" ".join(para_kept))
        
        # ── Phase 82.75: N-gram (10-word) Near-Dedup (Zero AI Cost) ──
        # Same but for n-grams
        word_sequences: dict[str, list[int]] = {}
        # Flat list for n-gram logic
        flat_sentences = []
        for p_idx, p in enumerate(final_paragraphs):
            p_sents = [s.strip() for s in re.split(r'(?<=[.!?。])[\s\n]+', p) if s.strip()]
            for s in p_sents:
                flat_sentences.append({"text": s, "p_idx": p_idx})

        for s_idx, s_obj in enumerate(flat_sentences):
            words = normalize_vn(s_obj["text"]).split()
            if len(words) < 10: continue
            for i in range(len(words) - 9):
                ngram = ' '.join(words[i:i + 10])
                if ngram not in word_sequences: word_sequences[ngram] = []
                word_sequences[ngram].append(s_idx)
        
        to_remove_s_indices: set[int] = set()
        for ngram, indices in word_sequences.items():
            u_indices = sorted(list(set(indices)))
            if len(u_indices) >= 2:
                for si in u_indices[1:]: to_remove_s_indices.add(si)
        
        if to_remove_s_indices:
            logs.append(f"✂️ Đã lọc {len(to_remove_s_indices)} câu có cụm từ trùng lặp (n-gram > 10 từ).")
            # Filter flat sentences and reconstruct paragraphs
            final_paragraphs_dedicated = [[] for _ in range(len(final_paragraphs))]
            for i, s_obj in enumerate(flat_sentences):
                if i not in to_remove_s_indices:
                    final_paragraphs_dedicated[s_obj["p_idx"]].append(s_obj["text"])
            
            final_paragraphs = [" ".join(p_s) for p_s in final_paragraphs_dedicated if p_s]

        cleaned_draft = "\n\n".join(final_paragraphs)
        logger.info(f"[PlagiarismCop] bulk_fix: Deterministically cleaned article structure.")
...
"""
        try:
                            ...
                            if len(norm_old) > 20: 
                                ...
                                for start_idx in range(len(final_content) - len(old_txt) + 20):
                                    ...
                                    if RE_WHITESPACE.sub('', window).startswith(norm_old):
                                        ...
                                        replacements_made += 1
                                        match_found = True
                                        msg = f"✅ Đã phẫu thuật xong: \"{old_txt[:30]}...\""
                                        logger.info(f"[PlagiarismCop] {msg}")
                                        logs.append(msg)
                                        break
            ...
            logs.append(f"🏅 Hoàn tất! Đã xử lý {replacements_made + (len(raw_paragraphs) - len(kept)) + dedup_count} điểm yếu.")
    ...
    async def analyze(self, campaign: ContentCampaign) -> PlagiarismResult:
        ...
        """
        logs = []
        logs.append("🔍 Khởi động hệ thống tầm soát bản quyền...")
        ...
        logs.append("🧹 Đang tiền xử lý nội dung bài viết...")
        ...
        logs.append("🧠 Đang rà soát trùng lặp nội bộ (Internal Dedup)...")
        ...
        logs.append(f"📡 Đang trinh sát 5 nguồn đối thủ hàng đầu cho: '{primary_keyword}'...")
        competitor_snippets = await self._fetch_competitor_snippets(primary_keyword, logs=logs)

        # 3. AI semantic analysis with per-sentence annotations
        logs.append("🧠 Đang nạp dữ liệu vào Neural Engine để thẩm định...")
        try:
            prompt = f"""
...
"""
            ...
            logs.append(f"✅ Thẩm định hoàn tất! Điểm độc nhất: {int(raw.uniqueness_score * 100)}%.")
            raw.logs = logs
            return raw
    ...
    async def _fetch_competitor_snippets(self, keyword: str, logs: Optional[List[str]] = None) -> List[str]:
        ...
        try:
            ...
            if logs is not None:
                logs.append(f"📡 Đã tải xong nội dung từ {len(snippets)} website đối thủ.")
