...
logger: logging.Logger = logging.getLogger("test-elite")
...
async def run_test_scenarios() -> None:
    ...
    await handler.handle(ctx)
    if ctx.order_draft and ctx.order_draft.customer_address:
    ...
    addr: str = "336/28/19 Nguyễn Văn Luông, Phú Lâm"
    ...
    mixed_msg: str = "về địa chỉ 336/28/19 nguyễn văn luông, phú lâm 0949901122"
    
    # Test bóc tách SĐT từ chuỗi lộn xộn
    digits_only: str = re.sub(r"\D", "", mixed_msg)
    phone_match: Optional[re.Match[str]] = re.search(r"0\d{9}", digits_only)
    phone: Optional[str] = phone_match.group() if phone_match else None
