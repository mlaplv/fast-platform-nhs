<script lang="ts">
  ...
  function selectTip(command: string) {
    playTick();
    vuiController.execTextCmd(command);
  }
</script>
...
{#if nanobot.showQuickTips}
  ...
  >
    <div
      class="px-5 py-4 border-b border-white/5 flex items-center justify-between bg-white/5"
    >
    ...
    <div class="p-2 max-h-[350px] overflow-y-auto custom-scrollbar">
      {#each nanobot.agenticSuggestions as suggestion}
        ...
        >
          <div
            class="text-[14px] font-medium text-white/90 group-hover:text-neon-cyan transition-colors"
          >
            {suggestion.label}
          </div>
          <div
