<script lang="ts">
  ...
  $effect(() => {
    ...
    if (isFocused || omni.cmd || isScrollingUp || pos < 20) {
...
>
  ...
  <div class="absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t from-black via-black/80 to-transparent pointer-events-none -z-10"></div>

  <div class="w-full px-1.5 pb-10 flex items-center justify-center gap-1.5 max-w-lg pointer-events-auto">
    <!-- Menu Button (Left) -->
    <button
    ...
    <div 
      class="flex-1 min-w-0 flex items-center gap-2 bg-black/70 backdrop-blur-[40px] border border-white/20 rounded-full px-4 py-1.5 shadow-[0_12px_48px_rgba(0,0,0,0.6)] transition-all duration-500 {isFocused ? 'border-neon-cyan/50 ring-1 ring-neon-cyan/20' : ''}"
      style:transform="scale({0.8 + ($scrollSpring * 0.2)})"
      style:max-width="{(0.4 + ($scrollSpring * 0.6)) * 100}%"
    >
      <div class="flex-1 flex items-center min-w-0" style:opacity={$scrollSpring > 0.3 ? 1 : 0}>
        <input
          bind:value={omni.cmd}
      ...
      <button
        onclick={omni.cmd ? () => omni.execCmd() : () => { playTick(); omni.toggleVoice(); }}
        class="w-9 h-9 shrink-0 flex items-center justify-center rounded-full transition-all duration-300 active:scale-90 {omni.cmd ? 'bg-neon-cyan/20 text-neon-cyan shadow-[0_0_15px_rgba(0,255,255,0.2)]' : omni.rec ? 'text-red-500 animate-pulse' : 'text-white/50'}"
      >
        {#if omni.cmd}
