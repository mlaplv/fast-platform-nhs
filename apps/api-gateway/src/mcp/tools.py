...
from src.database.models import Order, User, Draft, Notification, ProductBase
from src.mcp.protocol import mcp_registry
from shared.schemas.intent import IntentAction
from src.utils.data_stripper import DataStripper
...
async def search_products_semantic(db_session: AsyncSession, query: str, limit: int = 5):
    try:
        from src.services.product_vector_service import product_vector_service
...
async def search_articles_semantic(db_session: AsyncSession, query: str, limit: int = 5):
    try:
        from src.services.article_vector_service import article_vector_service
