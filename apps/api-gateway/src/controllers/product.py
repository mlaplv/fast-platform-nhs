...
from src.database.repositories import ProductBaseRepository, provide_product_repo
from src.database.models import ProductBase
from src.guards import PermissionGuard
from src.utils.sql import escape_like
from src.schemas.product import CreateProductRequest, UpdateProductRequest
from src.schemas.article import BulkIdsRequest
...
class ProductController(Controller):
    ...
    async def _upsert_embedding(self, prod_repo: ProductBaseRepository, product_id: str, name: str, description: Optional[str]):
        """Helper to generate and store pgvector embedding (R16)."""
        try:
            from ai_engine.core.vector_memory import get_encoder
            encoder = get_encoder()
            if not encoder:
                return
